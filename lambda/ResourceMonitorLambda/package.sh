...
if aws s3 ls $S3BucketName 2>&1 | grep -q 'NoSuchBucket'; then
    ...
    #aws s3api create-bucket --bucket $S3BucketName --create-bucket-configuration LocationConstraint="${S3BucketDeployRegion}"
    #if region is us-east-1, uncomment below line and comment above line
...
##change directory to zip file location here, else passs the path in below CLI##

# copying zip to S3 bucket
